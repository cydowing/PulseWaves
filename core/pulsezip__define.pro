...
dum = self->consoleclass::init(_extra = PARM)
...
if os.osType eq 'unix' then begin
  ...
  if keyword_set(WINE_PATH) then begin
    self.spawnCommand =WINE_PATH + os.sep + 'wine '
  endif else begin
    ; testing the presence of wine in the system - might no tbe 100% accurate
    spawn, '/usr/local/bin/wine --version', res, err
    if strlowcase(strmid(res, 0, 4)) eq 'wine' then self.print, 1, 'Wine seems installed on the system...' else begin
      self.print, 2, "Wine doesn't seems installed on the system..."
      self.print, 2, "If Wine is on your system, but not automatically detected, then use WINE_PATH keyword to specify the path to WINE..."
    endelse
    self.spawnCommand ='/usr/local/bin/wine '
  endelse
  
endif else begin
  ; We are on Windows - so nothing to do here...
endelse
...
if exist eq 1 then begin
  ...
  while exist ne 1 do begin
    ...
    if Keyword_set(NO_GUI) then begin
      self.print, 2, "No INPUT_FILE provided or the file doesn't exist..."
      self.print, 2, "Please re-enter a file path string"
      newPath = ""
      Read, newPath
      self.print, 1, newPath
      exist = File_test(newPath)
    endif else begin
      ; If not inputfile is provided, then open a dialog pickfile
      filters = ['*.pls', '*.plz']
      newPath = Dialog_pickfile(/READ, FILTER = filters)
      exist = File_test(newPath)
    endelse
  endwhile
...
if keyword_set(PULSEZIP_PATH) then begin
  ...
  cd, '..' + os.sep + 'dep'
  if os.osType eq 'unix' then spawn, 'pwd', res else spawn, 'cd', res
